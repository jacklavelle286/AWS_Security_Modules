...
data "aws_iam_policy_document" "cloudwatch_delivery_policy" {
  ...
  statement {
    ...
    resources = ["arn:aws:logs:${var.region}:${data.aws_caller_identity.current.account_id}:log-group:${aws_cloudwatch_log_group.cloudtrail_events[0].name}:log-stream:*"]
  }
...
resource "aws_cloudtrail" "global" {
  ...
  s3_bucket_name                = aws_s3_bucket.cloud-trail-baseline-bucket.bucket
...
resource "aws_s3_bucket" "cloud-trail-baseline-bucket" {
    force_destroy = var.force_destroy
}


resource "aws_s3_bucket_policy" "cloud-trail-policy" {
  bucket = aws_s3_bucket.cloud-trail-baseline-bucket.id
    ...
    Statement = [
      {
        ...
        Resource = [
          aws_s3_bucket.cloud-trail-baseline-bucket.arn,
          "${aws_s3_bucket.cloud-trail-baseline-bucket.arn}/*"
        ]
      }
    ]
